...
function ScrollBar({
  ...
  return (
    <ScrollAreaPrimitive.ScrollAreaScrollbar
      ...
      className={cn(
        'flex touch-none bg-[hsl(var(--scrollbar-track))] p-px transition-colors select-none',
        {
          'h-full w-2.5 border-l border-border/50': orientation === 'vertical',
          'h-2.5 flex-col border-t border-border/50': orientation === 'horizontal',
        },
        className,
      )}
      {...props}
    >
      <ScrollAreaPrimitive.ScrollAreaThumb
        ...
        className="relative flex-1 rounded-none bg-[hsl(var(--scrollbar-thumb))] transition-colors hover:bg-[hsl(var(--scrollbar-thumb-hover))]"
      />
    </ScrollAreaPrimitive.ScrollAreaScrollbar>
  )
}
